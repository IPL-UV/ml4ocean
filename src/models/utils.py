...
class PCATargetTransform(TransformedTargetRegressor):
    def __init__(self, regressor=None, n_components=10):
        ...
        super(PCATargetTransform, self).__init__(
            ...
            func-None, inverse_func=None, check_inverse=False
            )
    ...
    def predict(self, X, return_std=False, noiseless=True):
        ...
        if not return_std:
            preds = Parallel(n_jobs=self.n_jobs)(
                delayed(parallel_helper)(e, 'predict', X, return_std=return_std, noiseless=noiseless)
                for e in self.estimators_)
            
            return np.asarray(preds).T
        else:
            preds, stds = Parallel(n_jobs=self.n_jobs)(
                delayed(parallel_helper)(e, 'predict', X, return_std=return_std, noiseless=noiseless)
                for e in self.estimators_)
